...
class AbletonMCP(ControlSurface):
            ...
            # Commands that modify Live's state should be scheduled on the main thread
                ...
                def main_thread_task():
                    try:
                        ...
                        elif command_type == "load_browser_item_to_slot":
                            track_index = params.get("track_index", 0)
                            clip_index = params.get("clip_index", 0)
                            item_uri = params.get("item_uri", "")
    ...
    def _load_browser_item_to_slot(self, track_index, clip_index, item_uri):
        """Load a browser item onto a specific Session View clip slot."""
        try:
            if track_index < 0 or track_index >= len(self._song.tracks):
                raise IndexError("Track index out of range")
            if clip_index < 0 or clip_index >= len(self._song.scenes):
                raise IndexError("Clip index out of range")
            ...
            if hasattr(browser_or_item, 'instruments'):
                ...
                for attr in [
                    ...
                    if hasattr(browser_or_item, attr):
                        value = getattr(browser_or_item, attr)
